...
class TestOrderApiUnauthorisedUser(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.random_product_id_two = list(models.MainProductDatabase.objects.all())[
            1
        ].id
    ...
    def test_unauthorised_user_view(self) -> None:
        ...
        res = self.client.post(self.url_unauthorised, content_type="application/json")
        self.assertEqual(res.status_code, 200)

        res_data = json.loads(res.data)
        self.assertEqual(main_product.price, res_data["items"][0]["get_total"])
    ...
    def test_unauthorised_two_products(self) -> None:
        ...
        data = {
            "products": {
                str(self.random_product_id): {"quantity": 1},
        ...
        first_product_quantity = data["products"][str(self.random_product_id)][
            "quantity"
        ]
        second_product = models.MainProductDatabase.objects.get(
            id=self.random_product_id_two
        )
        second_product_quantity = data["products"][str(self.random_product_id_two)][
            "quantity"
        ]
