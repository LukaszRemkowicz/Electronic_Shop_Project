...
class LandingPageArticles(models.Model):
    ...
    alt_short_descript = models.CharField(
    ...
    img = models.ImageField(
        upload_to=f"articles/landin-page/{time.strftime('%Y-%m-%d')}",
        null=True,
        blank=True,
    )
    second_img = models.ImageField(
        upload_to=f"articles/landin-page/{time.strftime('%Y-%m-%d')}",
        null=True,
        blank=True,
    )
    third_img = models.ImageField(
        upload_to=f"articles/landin-page/{time.strftime('%Y-%m-%d')}",
        null=True,
        blank=True,
    )

    tempalate = models.FileField(
        upload_to=f"articles/blogs/{time.strftime('%Y-%m-%d')}",
        default="",
        null=True,
        blank=True,
    )
    ...
    def __str__(self) -> str:
        return f"Blog id: " f"{self.id}, posted: {self.posted} {self.title[:20]}.."
    ...
    def when_posted(self) -> str:
        ...
        return (
            f'Posted on {self.posted.strftime("%B")} '
