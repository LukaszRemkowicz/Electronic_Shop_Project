...
def return_url_params(request: HttpRequest) -> dict:
    ...
    return {
        ...
        if key != "page" and key != "grid" and key != "filter" and key != "ids"
    }
...
def filter_by_stars(products: QuerySet, value: str, model: QuerySet) -> QuerySet:
    ...
    products_list = [
        product.id for product in products if product.get_star_avg[0] == value
    ]
            ...
            if key == "stars":
                ...
                if value == "70' and more":
                    ...
                    products = products.filter(
                        Q(diagonal__lt=value[1]), Q(diagonal__gt=value[0])
                    )
            else:
    ...
    for key, value in url_queryset.items():
        try:
            if key == "stars":
                ...
                value = value.replace("'", "").split("-")
                products = Monitors.objects.filter(
                    Q(diagonal__lt=value[1]), Q(diagonal__gt=value[0])
                )
            else:
                products = filter[key](value, products)
    ...
    for key, value in url_queryset.items():
        try:
            if key == "stars":
                ...
                products = products.filter(
                    Q(screen_diagonal__lte=value[1]), Q(screen_diagonal__gte=value[0])
                )
...
def get_similar_products_data(request: HttpRequest):
    ...
    similar_products_ean = [
        try_to_get_product(product, "").ean for product in similar_products
    ]
    similar_products = get_model_queryset(similar_products[0].cattegory).filter(
        ean__in=similar_products_ean
    )
