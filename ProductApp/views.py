...
from ProductApp.utilss.view_utils import (
    ...
    filter_accesories_products,
    filter_cpus_products,
    filter_graphs_products,
    filter_headphones_products,
    filter_laptops_products,
    filter_monitors_products,
    filter_motherboard_products,
    filter_pcs_products,
    filter_pendrives_products,
    filter_phones_products,
    filter_rams_products,
    filter_routers_products,
    filter_ssd_products,
    filter_switches_products,
    filter_tv_products,
    get_all,
...
class ProductPage(ListView):
    ...
    def get_context_data(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        context["url_last"], context["url_path"] = get_url_path(
            self.request, product_page=True
        )
...
class ProductsCart(ListView):
    ...
    def get_context_data(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        if self.request.GET.get("ids"):
            ...
            params_list = [
                key for key, _ in self.request.GET.lists() if key not in params_excluded
            ]
            if params_list:
...
class QueryResult(ListView):
    ...
    def get_context_data(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        products = [
            try_to_get_product(product, "") for product in products.order_by("-created")
        ]
